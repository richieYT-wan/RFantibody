...
RESULTS_DIR=""                            # By default, outputs will be saved at <ROOT_DIR/outputs/<OUTPUT_NAME>/; this flag overrides it (eg --results-dir data/04_outputs/<run_id>/<jobname>)
...
Misc. Arguments:
  ...
  --results-dir     STR     Custom output directory to save the results (default: RFantibody/outputs/<autogenerated_run_name>/)
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --results-dir)
      [[ $# -ge 2 ]] || die "$1 requires a value"
      RESULTS_DIR="$2" # Overrides the default results directory if provided
      shift 2 
      ;;
    # RFdiffusion
    --n-designs)
...
if [[ -n RESULTS_DIR ]]; then
  OUTPUT_DIR="$RESULTS_DIR/${FILENAME}"
else
