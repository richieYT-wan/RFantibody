...
LABEL_RE = re.compile(
    r"^REMARK\s+PDBinfo-LABEL:\s*(\d+)\s+(\S+)\s*$")
SCORE_RE = re.compile(
    r"^SCORE\s+([^:]+):\s*(.+?)\s*$")
...
Dict[
    ...
    lines = read_lines(
        pdb)
    seqs = parse_sequences_from_pdb(
        pdb)
    labels = parse_labels(
        lines)
    scores = parse_scores_from_pdb(
        lines)
    ...
    Dict[
        str, Any] = {
        "id": pdb.stem,
        # identifier in RFantibody run, e.g. its ID in produced by RFantibody
        "vh": seqs.get(
            "H",
            ""),
        "t": seqs.get(
...
def parse_pdb_dir(
    ...
    if not pdbs:
        raise SystemExit(
            f"ERROR: No .pdb files found in: {pdb_dir}")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Parse RF2 output PDBs and scores into a single TSV/CSV.")
    p.add_argument(
        "--pdb-dir",
        required=True,
        type=Path,
        help="Directory containing output .pdb files")
    p.add_argument(
        "--original_filename",
        required=True,
        type=str,
    ...
    p.add_argument(
        ...
        required=True,
        type=Path,
        help="Output table (.tsv or .csv)")
    p.add_argument(
        "--scores",
        type=Path,
        default=None,
        help="Optional .sc scores file (qvscorefile output)")
    p.add_argument(
        '--framework',
        type=str,
        default=None,
        help="str identifier for the framework")
    p.add_argument(
        '--target',
        type=str,
        default=None,
        help="str identifier for the target")
    p.add_argument(
        '--hotspot',
        type=str,
        default=None,
        help="str identifier for the hotspots")
    p.add_argument(
        '--n_jobs',
        type=int,
        default=-1,
        help='Parallelisation of PDB parsing (default: -1)')
    return p.parse_args()
...
def main() -> None:
    ...
    if not pdb_dir.is_dir():
        raise SystemExit(
            f"ERROR: --pdb-dir is not a directory: {pdb_dir}")
    ...
    cols = list(
        df.columns)
    identifiers = parse_filename(
        args.original_filename)
    reorder = list(
        identifiers.keys()) + cols
